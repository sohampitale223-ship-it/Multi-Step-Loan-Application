...
function Step3KYC({ onBack, onContinue }) {
  ...
  return <form className="loan-form" onSubmit={handleSubmit(submit, focusFirstError)} noValidate>
    <div className="verification-field"><Controller name="panNumber" control={control} render={({ field }) => <MaskedInput {...field} id="pan-number" maskType="pan" label="PAN Number" helpText="Format: AAAAA9999A. Example test PAN: ABCPD1234F" autoComplete="off" error={errors.panNumber} required forceMasked={panVerification.isVerified} onFocus={() => { if (panVerification.isVerified) panVerification.reset() }} onChange={(event) => { panVerification.reset(); field.onChange(event) }} />} /><button className="button button--verify" type="button" disabled={panVerification.isVerifying} onClick={() => verifyField('panNumber', panVerification)}>Verify PAN</button></div>
